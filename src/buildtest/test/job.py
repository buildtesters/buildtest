...
def update_job_template(job_template):
    """ check if job template is valid when specified by --job-template option. Need to check if file exist and is valid extension"""
...
def generate_job(testpath,shell_type, jobtemplate, config):
    """ generate job script based on template file, shell type and path to test file """
