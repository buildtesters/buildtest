...
files and create test scripts along with the CMakeLists configuration. Each yaml file
will generate one test script
"""
...
def generate_source_test(configmap,codedir,subdir):
    """This function generates the tests that requires compilation for application. The
...
def get_environment_variable(shell,key, value):
    """ return environment variable for the corresponding shell type so it can be set in the test script"""
