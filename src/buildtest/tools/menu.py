...
"""
    ...
    parser_build_bsub.add_argument("-n", "--ntasks", help="Submits a parallel job and specifies number of tasks in job (bsub -n)", type=str)
    parser_build_bsub.add_argument("-m", "--machine", help="Submit job to specific hosts (bsub -m)", type=str)
    parser_build_bsub.add_argument("-W", "--walltime", help="Wall Time of Job (bsub -W)", type=str)
    parser_build_bsub.add_argument("-M", "--memory", help="Sets per-process (soft) memory for all process in job (bsub -M)", type=str)
    parser_build_bsub.add_argument("-J", "--jobname", help="Assign a Job Name (bsub -J)", type=str)

    parser_build_bsub.add_argument("--dry-run", help="Preview bsub command and not submit job to scheduler", action="store_true")
    ...
    parser_collection.add_argument("--check", help="Check all module collection by performing module load test.", action="store_true")
