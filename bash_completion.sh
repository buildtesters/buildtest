...
_buildtest ()
{
  ...
  local cmds="build buildspec cd cdash clean config docs edit help inspect history path report schema schemadocs systeminfo"
  ...
  case "$next" in
    build|bd)
      ...
      local longoption="--buildspec --disable-executor-check --executor --exclude --filter --helpfilter --max-pend-time --poll-interval --report --retry --stage --tags"
