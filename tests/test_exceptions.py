...
class TestBuildTestError:
    ...
    @pytest.mark.xfail(
        reason="Testing to see if exception of type BuildTestError is raised", raises=BuildTestError
    )
    def test_exception(self):
        ...
        err = a.get_error()
        ret = a.returnCode()
        raise BuildTestError(f"Command: {cmd} failed, Return Code: {ret} with Error: {err}")