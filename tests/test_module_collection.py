...
from buildtest.tools.modulesystem.collection import add_collection, list_collection, \
    clear_module_collection, remove_collection, update_collection, get_collection_length, check_module_collection, get_buildtest_module_collection
...
@pytest.mark.xfail(reason="When collection file is not found, we can't remove a module collection ",raises=BuildTestError)
def test_collection_remove_when_file_not_found():
    os.remove(BUILDTEST_MODULE_COLLECTION_FILE)
    remove_collection(0)

@pytest.mark.xfail(reason="When collection file is not found, we can't update a module collection ",raises=BuildTestError)
