...
Buildtest can cap a limit on number of tests that can run concurrently. This can be set in configuration file via :ref:`max_jobs <configuring_max_jobs>`_
...
.. drop-down:: ``buildtest build -b tutorials/hello_world.yml --rebuild=5 --max-jobs=2``
